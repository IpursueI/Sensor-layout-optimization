...
class DrawAverageRMSE:
    ...
    def draw(self, temp_hum):
        #title = ['Temperature interpolation average RMSE cross_validation','humidity interpolation average RMSE cross_validation']
        title = [u'温度均方根误差均值',u'湿度均方根误差均值']
        ...
        plt.ylabel(u'均方根误差均值', fontproperties = zhfont, fontsize = 15)
        ...
        #plt.show()
        if temp_hum == 0:
            plt.savefig(u'../data/result/温度均方根误差均值图.png')
        else:
            plt.savefig(u'../data/result/湿度均方根误差均值图.png')
