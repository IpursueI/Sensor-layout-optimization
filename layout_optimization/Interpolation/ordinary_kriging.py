...
class OrdinaryKriging:
    ...
    def __init__(self, filtered_file_path, pos_file_path, selected_sensors, unselected_sensors, each_sensor_number):
    ...
    def do_interpolate(self, data, grid_x, grid_y, grid_z, row_idx):
        #对温度进行插值
        ok3d_temp = OrdinaryKriging3D(data[:, 0], data[:, 1], data[:, 2], data[:, 3],variogram_model='spherical')
        #print data[:,1]
        #print gridx
        k3d_temp, ss3d_temp = ok3d_temp.execute('points', grid_x, grid_y, grid_z)
        
        #对湿度进行差值
        ok3d_hum = OrdinaryKriging3D(data[:, 0], data[:, 1], data[:, 2], data[:, 4],variogram_model='spherical')
