...
class RandomValidation:
    def __init__(self, sensor_count, filtered_file_path, pos_file_path, dis_file_path):
    ...
    def generate(self, group_count, member_count):
        result = []
        set_all = set(range(self.sensor_count))
        for i in range(group_count):
            item = []
            selected_list = random.sample(range(self.sensor_count), member_count)
            unselected_list = list(set_all.difference(set(selected_list)))
            selected_list = [self.sensor_num[j] for j in selected_list]
            unselected_list = [self.sensor_num[j] for j in unselected_list]
    ...
    def validation(self, group_count, member_count, validation_result_file_path, record_num):
        res = self.generate(group_count, member_count)
        final_error = []
        ...
        for item in res:
            ...
            ok3d_spherical = OrdinaryKriging(self.filtered_file_path,self.pos_file_path,selected_sensors,unselected_sensors, record_num,0, 'spherical')
            ...
            ok3d_power = OrdinaryKriging(self.filtered_file_path,self.pos_file_path,selected_sensors,unselected_sensors, record_num,0, 'power')
            ...
            ok3d_exponential = OrdinaryKriging(self.filtered_file_path,self.pos_file_path,selected_sensors,unselected_sensors, record_num,0, 'exponential')
...
if __name__ == "__main__":
    rv = RandomValidation(34, "../data/filter_data","../data/pos/pos.csv","../data/pos/distance.csv")
    #print rv.generate(3,10)
    rv.validation(20, 10, ["../data/result/RMSE.csv", "../data/result/MeanError.csv", "../data/result/Pearson.csv"],10)
    print "done"